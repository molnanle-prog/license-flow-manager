...
const DEFAULT_CONFIG: AppConfig = {
  ...
  programs: [
    ...
    { id: 'ezprintwork-program', programId: PROGRAM_IDS.EZPRINTWORK, name: 'EzPrintWork 관리', sheetId: '1DBSYg8Lqp-Z0o4e35vGsU00XhJeClua-cirsH32xRFQ', productName: 'EzPrintWork', gasUrl: '', securityToken: 'EzImpo_Secure_Handshake_Token_v3_X9Z' }
  ],
...
export const getAppConfig = (): AppConfig => { const cfg = localStorage.getItem(STORAGE_KEYS.APP_CONFIG); if (cfg) { try { const s = JSON.parse(cfg); const m = { ...DEFAULT_CONFIG, ...s }; DEFAULT_CONFIG.programs.forEach(d => { if (!m.programs.some((p:any)=>p.id===d.id)) m.programs.push(d); }); return m; } catch (e) {} } localStorage.setItem(STORAGE_KEYS.APP_CONFIG, JSON.stringify(DEFAULT_CONFIG)); return DEFAULT_CONFIG; };
...
const parseKoreanDate = (dateStr: any): string => {
    if (!dateStr) return '';
    try {
