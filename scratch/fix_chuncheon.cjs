...
async function run() {
    ...
    try {
        ...
        const newRows = rows.map((row) => {
            // Check if the row contains chuncheon_new@gmail.com and companyName is "춘천인새"
            if (row[0] === 'chuncheon_new@gmail.com' && row[6] === '춘천인새') {
                console.log(`Found row: ${row.join(' | ')}`);
                row[6] = '춘천인쇄'; // Fix the typo!
                console.log(`Updated to: ${row.join(' | ')}`);
                updated = true;
            }
            return row;
        });
        ...
        if (updated) {
            ...
            console.log("Successfully fixed the typo on Google Sheets!");
        } else {
            console.log("Chuncheon row not found or already correct.");
        }
    } catch (e) {
