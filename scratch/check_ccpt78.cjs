...
const app = initializeApp(firebaseConfig, 'checkCcpt78App');
const db = getFirestore(app, firebaseConfig.firestoreDatabaseId);

async function run() {
  console.log("Searching Firestore 'users' for email ccpt78@gmail.com...");
  const usersRef = collection(db, 'users');
  const q = query(usersRef, where('email', '==', 'ccpt78@gmail.com'));
  const snap = await getDocs(q);
  console.log(`Found ${snap.size} user documents for ccpt78@gmail.com:`);
